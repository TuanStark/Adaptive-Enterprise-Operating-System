...
import { ProcessApprovalCommand, ProcessApprovalHandler } from '../../application/commands/process-approval/process-approval.handler';
...
export class ApprovalController {
  ...
  async create(@Body() dto: CreateApprovalRequestDto, @Req() req: Request) {
    ...
    const command = new CreateApprovalCommand(
      dto.tenantId, dto.workspaceId, user.userId, dto.title,
      dto.entityType, dto.entityId, dto.reviewerIds,
    );
  ...
  async list(@Query('workspaceId') workspaceId: string, @Query('page') page?: string, @Query('limit') limit?: string) {
    ...
    return {
      data: data.map((a) => ({
        id: a.id, title: a.title, status: a.status, requesterId: a.requesterId,
        steps: a.steps.map(s => ({ reviewerId: s.reviewerId, status: s.status, comment: s.comment })),
  ...
  @Patch(':id/process')
  async process(@Param('id') id: string, @Body() dto: ProcessApprovalRequestDto, @Req() req: Request) {
    const user = (req as any).user;
    const result = await this.processHandler.execute(new ProcessApprovalCommand(id, user.userId, dto.action, dto.comment));
    if (result.isFail) throw result.error as DomainError;
