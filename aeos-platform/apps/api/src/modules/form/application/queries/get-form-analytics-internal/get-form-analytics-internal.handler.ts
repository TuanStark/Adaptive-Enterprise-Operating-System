...
import { GetFormAnalyticsInternalQuery, FormAnalyticsDto } from '../../../../../common/contracts/form.contract';
...
export class GetFormAnalyticsInternalHandler implements IQueryHandler<GetFormAnalyticsInternalQuery> {
  ...
  async execute(query: GetFormAnalyticsInternalQuery): Promise<FormAnalyticsDto> {
    const totalForms = await this.prisma.dynamicForm.count({ where: { workspaceId: query.workspaceId } });
    return { totalForms };
  }
}
