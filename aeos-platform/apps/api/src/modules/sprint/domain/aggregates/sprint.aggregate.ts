...
export class Sprint extends AggregateRoot<string> {
  ...
  private constructor(
    ...
    goal: string | null, status: SprintStatus, startDate: Date | null,
    endDate: Date | null, version: number, createdAt?: Date, updatedAt?: Date,
  ) {
  ...
  static create(
    tenantId: string, projectId: string, name: string,
    goal: string | null, startDate: Date | null, endDate: Date | null,
    ...
    return Result.ok(new Sprint(
      id, tenantId, projectId, name.trim(), goal,
      SprintStatus.PLANNING, startDate, endDate, 0,
    ));
  }

  static fromPersistence(props: SprintProps): Sprint {
    return new Sprint(
      props.id, props.tenantId, props.projectId, props.name,
      props.goal, props.status, props.startDate, props.endDate,
      props.version, props.createdAt, props.updatedAt,
    );
  }
  ...
  complete(): Result<void, InvalidSprintStatusTransitionError> {
    if (this._status !== SprintStatus.ACTIVE) {
      return Result.fail(new InvalidSprintStatusTransitionError(this._status, SprintStatus.COMPLETED));
    }
