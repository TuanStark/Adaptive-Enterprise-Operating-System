...
export class DocumentController {
  ...
  async update(@Param('id') id: string, @Body() dto: UpdateDocumentRequestDto) {
    const result = await this.updateHandler.execute(
      new UpdateDocumentCommand(id, dto.name, dto.visibility),
    );
    if (result.isFail) throw result.error as DomainError;
  ...
  async publishVersion(@Param('id') id: string, @Body() dto: PublishVersionRequestDto) {
    const result = await this.publishVersionHandler.execute(
      new PublishDocumentVersionCommand(id, dto.fileId),
    );
    if (result.isFail) throw result.error as DomainError;
